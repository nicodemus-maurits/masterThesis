...
/*	if ( (red_lp_queue.queue_tail+1 == red_lp_queue.queue_head) ||
	     ( (red_lp_queue.queue_tail == MAX_NUM_RED_IN_SK-1) && (red_lp_queue.queue_head == 0) ))
	{
		return false;
	}*/
...
bool rmptcp_calc_lowpass_timeout(struct sock *sk)
{
	...
	skb_queue_walk_safe(&sk->sk_receive_queue, skb, tmp) {
		...
		else
		{
			...
			if (delta_mean != 0)
			{
				delta_mean_max = delta_mean + ((delta_mean/100)*sysctl_lowpass_learn_rate);
	...
	if (!any_hit)
	{
		...
		if(delta_mean > sysctl_lowpass_max_wait * MICRO2NS) // 1000 => conversion micro s to ns
		{
			delta_mean = sysctl_lowpass_max_wait * MICRO2NS;
		}
